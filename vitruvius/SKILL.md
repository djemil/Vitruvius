...
3. **Doctrine contradictions** — the project's own rules, against each other and against the code.
   ...
   The third is not a contradiction, so it is reported as its own list rather than as conflict blocks, and it is the one that quietly decides whether the rulebook is load-bearing. Prose is the weakest enforcement a project has: it works only when a future agent loads the rule, reads it, and remembers. A numeric invariant, a layering constraint, a forbidden import, a shape or schema rule can all be *checked* instead — fired at the moment of violation, needing nobody to have read anything, unable to drift from the code.

   **Sort every rule into four states, not two.** The distinction that matters is between a rule nothing enforces and a rule whose enforcement exists but is invisible:

   - **Enforced and cited** — a test, type, schema or build check exists AND names the rule. Nothing to do.
   - **Enforced but uncited** — the check exists; the enforcing code never mentions the rule ID. The rule is doing real work and looks like dead prose. The fix is one comment, not a new test: cite the ID where the check lives, so the next reader can see the rule is load-bearing and the next auditor does not propose rebuilding what already exists.
   - **Unenforced but convertible** — the subject matter is checkable and no check exists. Name the specific mechanism that would replace it.
   - **Irreducibly prose** — intent, process discipline, judgment, and negative rules about things absent from the codebase. Group by which, because this set is what must stay prose and it is usually much smaller than the whole rulebook.

   **Do not measure this with a grep for rule IDs in source.** That counts citations, not enforcement, and the two are not the same: it cannot distinguish *enforced but uncited* from *unenforced*, which are opposite problems with opposite remedies. Establishing which bloc a rule is in means looking for the check, not for the ID. This exact error was made at S136 — a grep found 1 of GRIDIGMA's 338 rules referenced in source and was read as "one rule is enforced", when its own survey then found fifty-plus already enforced by gates, boundary tests and shape caps whose code simply carried no ID. Report all four counts; a single ratio hides the finding.
