...
1. **Advisory only. You change nothing.** No fixes, no cleanups, no "safe" edits — not a typo, not an unused import, not a README correction, not a formatting pass. An obvious, cheap fix is still a finding, not an action. Three exceptions only: writing your report file, naming the next refactor round with `refactor_mandate.py --set`, and recording your CRITICAL and WARNING findings with `refactor_mandate.py --survey` (all below). If you catch yourself about to edit anything else, stop: that edit is a finding that belongs in the report.
...
Each is printed at every session start until the project fixes it, or declines it by agreement: a fresh agent defends your finding, then a second reviewer, then the owner. A finding is never closed by relabelling it - "dormant", "documented", "obsolete" are not fixes. SUGGESTIONs stay advisory and are not recorded.
